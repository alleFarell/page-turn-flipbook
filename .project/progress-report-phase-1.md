...
- [ ] **3.1 Slider Mode (`SliderViewer.tsx`)**: Build a custom horizontal flex container with strict CSS `scroll-snap`.
- [ ] **3.2 Cards Mode (`CardsViewer.tsx`)**: Build an absolute-positioned stacked deck with swipe-away animations.
- [ ] **3.3 Coverflow Mode (`CoverflowViewer.tsx`)**: Build a 3D CSS carousel with Y-axis rotation and depth scaling.

### 4. Aesthetic Polish & Design Quality
*Applying the `frontend-design` skill guidelines.*
- [ ] **4.1 Atmosphere**: Apply dynamic lighting gradients, noise textures, and deep shadows tied to the user's color config.
- [ ] **4.2 Micro-interactions**: Add crisp hover states, transitions, and staggered reveal animations to the toolbar and viewer shell.
- [ ] **4.3 Verification**: Test all modes against mobile responsiveness and cross-browser rendering.
